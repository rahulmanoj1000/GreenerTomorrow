...
def completed(request, citizen_id):
    if request.method == "POST":
        ...
        worker_complete = Progress.objects.get(pk=citizen_id)
        print(citizen_id)
        print (worker_complete)
        a = Pre_completed.objects.create(
        ...
        Completed.objects.create(sent_id=a.id,image = image)
        worker_complete.delete()
        return redirect('middleman:workman_panel')
...
def deleting(request, id):
    if id:
        ...
        mail = EmailMessage(
        'Pollution Report',
        f'The location you reported on {pre_complete.input_date} with regards to pollution/littering has been cleared. A picture post-clearance has been attached for your reference. Please feel free to drop a review at http://127.0.0.1:8000/rating/ratings/',
